...
declare
    ...
    customer_pb integer;
begin
    select full_name into actor_name from public.profiles where id = auth.uid() and active = true;
    ...
    values (p_date, p_type, p_pb_number, p_customer_id, p_amount, auth.uid(),
        case when p_type = 'cashIn' then actor_name else null end,
        case when p_type = 'cashOut' then actor_name else null end)
...
begin
    ...
    set date = p_date, pb_number = p_pb_number, customer_id = p_customer_id,
        amount = p_amount,
        received_by = case when existing.type = 'cashIn' then actor_name else null end,
        issued_by = case when existing.type = 'cashOut' then actor_name else null end
    where id = p_id
    returning * into result;
    insert into public.audit_log(actor_id, action, entity, entity_id)
